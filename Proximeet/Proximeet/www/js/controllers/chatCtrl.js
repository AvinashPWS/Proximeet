angular.module('ionicApp.chatCtrl', []);


angular.module('ionicApp.chatCtrl', ['ionicApp.elastic', 'angularMoment'])


        .controller('chatCtrl', ['$scope', '$rootScope', '$state',
            '$stateParams', 'MockService', '$ionicActionSheet',
            '$ionicPopup', '$ionicScrollDelegate', '$timeout', '$interval', '$ionicHistory', '$ionicPlatform', 'shareDetail', 'Crud', 'userLogin', 'socket', 'hiLightService',
            'chatData',
            function ($scope, $rootScope, $state, $stateParams, MockService,
                    $ionicActionSheet,
                    $ionicPopup, $ionicScrollDelegate, $timeout, $interval, $ionicHistory, $ionicPlatform, shareDetail, Crud, userLogin, socket, hiLightService, chatData) {
                ...
                $scope.AdminChatFlag = (userLogin.user_id === $scope.AdminID) ? 1 : 0;

                $scope.AdminApp_ID = 1;

                $scope.AdminChat = chatData.getChatObj()["AdminChat"] ? true : false;

                function checkUser() {

                    if (userLogin.user_id == chatData.getChatObj()["provider_id"]) {
                        return 0;
                    } else {
                        return 1;
                    }
                ...
                function getSubId() {
                    if (shareDetail.inituscriber) {
                        return shareDetail.inituscriber
                    } else {
                        return shareDetail.postDetail.Subscriber.subscriber_id
                    }
                }

                /*socket.emit('Viewed', {
                 subscriber_id: chatData.getChatObj()["subscriber_id"],
                 provider_id: chatData.getChatObj()["provider_id"],
                 flag: checkUser()
                 }, function (res) {
                 
                 });*/

                console.log(chatData.getChatObj()["subscriber_id"]);
                console.log(chatData.getChatObj())


                function getStrToArr(a) {
                    return a.split(",");
                }

                $scope.oldMsg = "";

                function pushNewMessage(arr, name) {
                ...
                $rootScope.$on('rootScope:pushNewMessage', function (event, data) {
                    if ($scope.oldMsg != data.additionalData.msg1) {

                        if (data.additionalData["provider_id"] == chatData.getChatObj()["provider_id"] && chatData.getChatObj()["post_id"] == data.additionalData.post_id) {
                            $scope.msg = data.additionalData.msg1;
                            pushNewMessage(data.additionalData.msg1.split(","), data.additionalData.name);
                            $scope.makeViewed()
                        } else {
                            if (chatData.ensureSameSubscriber(data.additionalData.subscriber_id)) {
                                console.log("subdata", data.additionalData.msg1);
                                $scope.msg = data.additionalData.msg1;
                                pushNewMessage(data.additionalData.msg1.split(","), data.additionalData.name);
                                $scope.makeViewed();
                            }
                ...
                socket.on('new message', function (data) {
                    ...
                    console.log("new message init", data)
                    function pushNewMessage() {
                        var messageObj = {};
                        //messageObj.date = new Date(val.Chat.time);
                        messageObj.username = $scope.toUser.username;
                        messageObj.userId = $scope.toUser._id;
                        messageObj.pic = $scope.toUser.pic;
                        messageObj.text = data.msg;
                        $scope.messages.push(messageObj);
                        viewScroll.scrollBottom(true);
                        viewScroll.resize();
                    }
                    //if provider check for post id
                    if (chatData.getChatObj()["provider_id"] == localStorage.user_id && chatData.getChatObj()["post_id"] == data.post_id && chatData.getChatObj()["subscriber_id"] == data.subscriber_id) {
                        pushNewMessage();
                        $scope.makeViewed();
                    } else {
                        if (chatData.ensureSameSubscriber(data.subscriber_id)) {
                            pushNewMessage();
                            $scope.makeViewed();
                        }
                    }
                ...
                socket.on('new admin message', function (data) {
                    ...
                    if (chatData.getChatObj()['To'] === data['nick']) {
                        pushNewMessage();
                        $scope.makeViewed();
                    }
                ...
                if (localStorage.user_id == chatData.getChatObj()["provider_id"]) {
                    $scope.toUser = {
                        _id: '0',
                        //534b8fb2aa5e7afc1b23e69c
                        pic: 'img/default_usera.png',
                        username: chatData.getChatObj()["subscriber_name"]
                    };

                    // this could be on $rootScope rather than in $stateParams
                    $scope.user = {
                        _id: '1',
                        //534b8e5aaa5e7afc1b23e69b
                        pic: 'img/default_usera.png',
                        username: chatData.getChatObj()["provider_name"]
                    };
                } else {
                    $scope.toUser = {
                        _id: '1',
                ...
                function getMessages() {
                    ...
                    if ($scope.AdminChat) {
                        ...
                        sRootURL = "chats/adminUserConversation.json";
                        sRequestJSON = {"logged_in_user": userLogin.user_id, "user_id": chatData.getChatObj()['user_id'], "admin_id": $scope.AdminID};

                    } else {
                        sRootURL = "chats/conversation.json";
                        sRequestJSON = {
                            "subscriber_id": chatData.getChatObj()["subscriber_id"],
                            "provider_id": chatData.getChatObj()["provider_id"],
                    ...
                    Crud.create({
                        root: sRootURL
                    }, sRequestJSON, function (data) {

                        console.log("old messages", data);

                        if ($scope.AdminChat) {
                            angular.forEach(data.result, function (val, index) {
                                var messageObj = {};
//                                messageObj.date = new Date(val.AdminChat.time);
                                messageObj.username = $scope.user.username;
                                messageObj.userId = val.AdminChat.flag;
                                messageObj.pic = $scope.user.picture;
                                messageObj.text = val.AdminChat.message;
                                $scope.messages.push(messageObj);

                            });

                        } else {
                            angular.forEach(data.response, function (val, index) {
                                var messageObj = {};
                                //messageObj.date = new Date(val.Chat.time);
                                messageObj.username = $scope.user.username;
                                messageObj.userId = val.Chat.flag;
                                messageObj.pic = $scope.user.picture;
                                messageObj.text = val.Chat.message;
                                $scope.messages.push(messageObj);
                ...
                $scope.sendMessage = function (sendMessageForm) {
                    ...
                    function checkFlag() {
                        if (userLogin.user_id == shareDetail.postDetail.Post.provider_id) {
                            localFlag = 1;
                            //owner
                            $scope.showOptionsOwner = true;
                        } else {
                            localFlag = 0;
                            //user
                            $scope.showOptionsOwner = false;
                        }
                    ...
                    if (chatData.getChatObj()["AdminChat"]) {
                        sEmitLink = "send admin message";
                        finalMessage.admin_id = $scope.AdminID;
                        finalMessage.app_id = $scope.AdminApp_ID;
                        finalMessage.user_id = chatData.getChatObj()["user_id"];
                        finalMessage.message = $scope.input.message;
                        finalMessage.flag = $scope.AdminChatFlag;

                    } else {
                        sEmitLink = "send message";
                        finalMessage.post_id = chatData.getChatObj()["post_id"];
                        finalMessage.app_id = chatData.getChatObj()["app_id"];
                        finalMessage.user_id = chatData.getChatObj()["user_id"];
                        finalMessage.subscriber_id = chatData.getChatObj()["subscriber_id"];
                        finalMessage.provider_id = chatData.getChatObj()["provider_id"];
                        finalMessage.message = $scope.input.message;
                        finalMessage.flag = chatData.getChatObj()["user_flag"];
                    }
                    ...
                    socket.emit(sEmitLink, finalMessage, function (data) {
                    ...
                    messageObj._id = new Date().getTime(); // :~)
                    messageObj.date = new Date();
                    messageObj.username = $scope.user.username;
                    messageObj.userId = $scope.user._id;
                    messageObj.pic = $scope.user.picture;

                    messageObj.text = finalMessage.message,
                            $scope.messages.push(messageObj);
                    viewScroll.scrollBottom(true);
                    viewScroll.resize();
                };
                ...
                $scope.acceptUser = function () {
                    ...
                    var acceptObj = {};
                    acceptObj.post_id = shareDetail.postDetail.Post.post_id;
                    acceptObj.app_id = shareDetail.postDetail.Post.app_id;
                    acceptObj.subscriber_id = subscriber_id_local;
                    acceptObj.user_id = shareDetail.postDetail.Post.post_actual_id;
                ...
                $scope.rejectUser = function () {
                    var rejectObj = {};
                    rejectObj.post_id = shareDetail.postDetail.Post.post_id;
                    rejectObj.app_id = shareDetail.postDetail.Post.app_id;
                    rejectObj.subscriber_id = subscriber_id_local;
                    rejectObj.user_id = shareDetail.postDetail.Post.post_actual_id;
...
angular.module('ionicApp.elastic', [])
