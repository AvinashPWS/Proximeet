angular.module('ionicApp.searchCtrl', ['ngAnimate'])
    .controller('searchCtrl', function (shareDetail, $scope, userLogin, $rootScope, $ionicPopup, $templateCache, $ionicPlatform, $ionicHistory, $ionicModal, $state
        ...
        //            $scope.drawMapPlotCircle = function () {
        ...
        //            };
        //            
        //            if(localStorage.searshListVisit){
        //                $scope.drawMapPlotCircle();
        //            }
        ...
        $scope.clearRecent = function () {
            ...
            $scope.filterObj.addressObj = undefined;
        };
        ...
        $scope.applyFilter = function () {
            ...
            if (!$scope.filterObj.addressObj) {
                ...
                $scope.updateCurrentPosition(nav_options);
                return;
            }
        ...
        $scope.getLocationAddress = function () {
            ...
            $scope.clearCategoryField = function ($event) {
                debugger;
        ...
        $scope.callInit = function () {
            ...
            if (sessionStorage.hasOwnProperty("zoom") && sessionStorage['zoom'] !== "undefined") {
                $scope.mapZoom = parseInt(sessionStorage['zoom']);
            }
        ...
        $scope.markCurrentLocation = function () {
            if (localStorage.hasOwnProperty("recent") === true && localStorage.recent != '') {
                localStorage.recent = '';
            }
            $scope.filterObj.addressObj = undefined;
        ...
        $scope.updateCurrentPosition = function (nav_options) {
        ...
        function makePostCall() {
            ...
            Crud.create({ root: "categories/list_view.json" }, sendSearch, function (response) {
                ...
                if (response.status) {
                    ...
                    if ($scope.AdminID === userLogin.user_id) {
                        updatedArray = response.result.slice(0, response.result.length - 1);
                    } else {
                        updatedArray = response.result;
                    }
                    response.result = updatedArray;
    ...
    .directive('clearelement', function () {
        return {
            link: function postLink($scope, element, attrs) {
                element.on("click", function (e) {
                    $scope.data.category.length = 0;
                })
            }
        }
    })
