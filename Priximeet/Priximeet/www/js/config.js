...
//            $rootScope.ROUTES = "http://78.47.154.117/greendiamond/";
            ...
            $rootScope.ROUTES = "http://78.47.154.117/production/proximeet/";
            $rootScope.ROUTES = "http://78.47.154.117/proximeet/";
            ...
            $rootScope.gotoDetail = function (postDetail) {
                shareDetail.postDetail = postDetail;
                $state.go('green.detail', {post_id: postDetail.post_id});
            ...
            $rootScope.onAdminNewMessageNotification = function (sNotificationDetails) {
                if (userLogin.user_id === localStorage.AdminID) {
                    chatData.setChatObj({
                        "To": sNotificationDetails['user_id'],
                        "subscriber_id": sNotificationDetails['user_id'],
                        "provider_id": userLogin.user_id,
                        "post_id": "",
                        "user_id": sNotificationDetails['user_id'],
                        "app_id": "1",
                        "flag": "1",
                        "provider_name": "Me",
                        "subscriber_name": sNotificationDetails.name,
                        "AdminChat": true
                    });
                } else {
                    chatData.setChatObj({
            ...
            $rootScope.onUserNewMessageNotification = function (sNotificationDetails) {
                //If logged in user is provider
                if (sNotificationDetails.provider_id === userLogin.user_id) {
                    chatData.setChatObj({
                        "subscriber_id": sNotificationDetails.subscriber_id,
                        "provider_id": sNotificationDetails.provider_id,
                        "post_id": sNotificationDetails.post_id,
                    ...
                    chatData.setChatObj({
                        ...
                        "app_id": '1',
                        "user_flag": 0,
                        "provider_name": sNotificationDetails.name,
                        "subscriber_name": "Me"
                    });
                }
            };
            ...
            $rootScope.onNewMessageOkClick = function (notification) {
                if (notification && notification['additionalData']) {
                    var sNotificationDetails = notification['additionalData'];
                    if (sNotificationDetails['admin_id']) {
                        $rootScope.onAdminNewMessageNotification(sNotificationDetails);

                    } else {
                        $rootScope.onUserNewMessageNotification(sNotificationDetails);
                    }
                    $state.go("green.chat");
                }
            };
...
//                    "android": {"senderID": "proximeet-1002", "icon": "www/image/ionic.png", "iconColor": "grey"},
                ...
                push.on('error', function (data) {
                    console.log("push >> registration >> error >> ", data);
                });
                    ...
                    setTimeout(function () {
                        switch (notification.title) {
                            ...
                            case "Event Start":
                                ...
                                $cordovaDialogs.confirm(notification.message, notification.title, ['Detail', 'Cancel'])
                                        .then(function (buttonIndex) {
                                            if (buttonIndex === 1) {
                                                $rootScope.gotoDetail(notification.additionalData);
                                ...
                                if (notification.additionalData.provider_id == localStorage.user_id) {
                                    ...
                                    if ($state.current.name == "green.chat") {
                                        ...
                                        if (notification.additionalData.post_id == chatData.getChatObj()["post_id"] && notification.additionalData.subscriber_id == chatData.getChatObj()["subscriber_id"]) {
                                            ...
                                            $cordovaDialogs.alert(notification.message, notification.title, 'OK')
                                                    .then(function () {
                                                        $rootScope.onNewMessageOkClick(notification);
                                                    });
